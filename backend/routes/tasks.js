...
// Create task
router.post('/', (req, res) => {
  const { columnId, title, description, position, priority, assignee, due_date, labels } = req.body;
  if (!columnId || !title) return res.status(400).json({ error: 'Column ID and title are required' });

  const metadata = { priority, assignee, due_date, labels };
...
router.put('/:id', (req, res) => {
  const { title, description, columnId, position, priority, assignee, due_date, labels } = req.body;
  const metadata = { priority, assignee, due_date, labels };
  Tasks.update(req.params.id, title, description, columnId, position, metadata, (err) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json({ id: req.params.id, title, description, columnId, position, priority, assignee, due_date, labels });
  });
});
